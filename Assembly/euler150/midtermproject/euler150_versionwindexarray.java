...
public class euler150
{
	public static void main(String [] args)
	{
		...
		int [] minArr = new int[inputArray.length-1];		
		int [] startIndices = new int[depth];
		
		for(int x=1; x<startIndices.length; x++)
			startIndices[x] = startIndices[x-1]+x;
		...
		{
			for(int j = 0; j < i+1 ; j++) //element is at i,j where i is the row number and j is each element in row i
			{
				int IJ = startIndices[i]+j+1;
